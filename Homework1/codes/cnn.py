...
class ConvNet(object):
  ...
  def __init__(self, input_dim=(1, 28, 28), num_filters=32, filter_size=7,
    ...
    W2 = np.random.normal(scale = weight_scale, size = (int(num_filters * H_prime * W_prime / 4), hidden_dim))
  ...
  def loss(self, X, y=None):
    ...
    out4, cache4 = fc_forward(out3_flat, W2, b2)
    ...
    dout5, dW3, db3 = fc_backward(dout6, cache6)
    dout4 = relu_backward(dout5, cache5)
    dout3_flat, dW2, db2 = fc_backward(dout4, cache4)
    dout3 = dout3_flat.reshape(out3.shape)
    dout2 = max_pool_backward(dout3, cache3)
    dout1 = relu_backward(dout2, cache2)
    dx, dW1 = conv_backward(dout1, cache1)
