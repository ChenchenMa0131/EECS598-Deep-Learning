...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        if self.cell_type == 'rnn':
          h, rnn_cache = rnn_forward(np.swapaxes(x,0,1), h0, Wx, Wh, b)
          h = np.swapaxes(h,0,1)

        elif self.cell_type == 'lstm':
          h, lstm_cache = lstm_forward(np.swapaxes(x,0,1), h0, Wx, Wh, b)
          h = np.swapaxes(h,0,1)

        # (4)
        out, out_cache = temporal_fc_forward(h, W_vocab, b_vocab)
        ...
        # Gradients
        # (4)
        dh, grads['W_vocab'], grads['b_vocab'] = temporal_fc_backward(dout, out_cache)
        ...
        # (1)
        dfeatures, grads['W_proj'], grads['b_proj'] = fc_backward(dh0, h0_cache)
    ...
    def sample(self, features, max_length=30):
        ...
        for t in range(max_length):
          word_embed,_ = word_embedding_forward(x, W_embed)
          ...
          out,_ = temporal_fc_forward(next_h.reshape((N,1,-1)), W_vocab, b_vocab)
          x = np.argmax(np.squeeze(out), axis = 1)
          captions[:,t] = x
